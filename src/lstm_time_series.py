...
def normalise_windows(window_data):
    ...
    for win in window_data:
        normalised_window = [((float(p) / float(win[0])) - 1) for p in win]
...
def predict_sequence_full(model, data, window_size):
    """Shift window by 1 new prediction each time, predict on new window."""
    curr_frame = data[0]
    predicted = []
...
def predict_sequences_multiple(model, data, window_size, prediction_len):
    """Predict sequence of 50 steps before shifting prediction run 50 steps."""
    prediction_seqs = []
...
if __name__ == '__main__':
    ...
    print('> Loading data... ')
    X_train, y_train, X_test, y_test = load_data('src/sp500.csv',
                                                 seq_len,
                                                 True)
